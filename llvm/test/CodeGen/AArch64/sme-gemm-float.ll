...
define dso_local i32 @GEMM(float* %matA, float* %matB, float* nocapture %matC, i32 %M, i32 %N, i32 %K) local_unnamed_addr #0 {
entry:
  ;CHECK-LABEL: GEMM:
  ;CHECK: smstart za
  ;CHECK-DAG: cntw    x{{[0-9]+}}
...
for.body:                                         ; preds = %for.body.lr.ph, %for.cond.cleanup7
  ; CHECK_DAG: whilelo p{{[0-9]+}}.s, x{{[0-9]+}}, x{{[0-9]+}}
  %i.0174 = phi i64 [ 0, %for.body.lr.ph ], [ %add82, %for.cond.cleanup7 ]
  %1 = call <vscale x 4 x i1> @llvm.aarch64.sve.whilelo.nxv4i1.i64(i64 %i.0174, i64 %conv)
...
for.body8:                                        ; preds = %for.body8.lr.ph, %cleanup75
  ; CHECK-DAG: whilelo p{{[0-9]+}}.s, x{{[0-9]+}}, x{{[0-9]+}}
  ; CHECK-DAG: zero    {za{{[0-9]+}}.s}
  %j.0170 = phi i64 [ 0, %for.body8.lr.ph ], [ %add78, %cleanup75 ]
  %3 = call <vscale x 4 x i1> @llvm.aarch64.sve.whilelo.nxv4i1.i64(i64 %j.0170, i64 %conv4)
  %4 = call <vscale x 16 x i1> @llvm.aarch64.sve.convert.to.svbool.nxv4i1(<vscale x 4 x i1> %3)
  %5 = call <mscale x 16 x float> @llvm.aarch64.sme.zero.mxv16f32()
  br i1 %cmp12158.not, label %for.cond56.preheader, label %for.body15.lr.ph

for.body15.lr.ph:                                 ; preds = %for.body8
  %add.ptr45 = getelementptr inbounds float, float* %matB, i64 %j.0170
  br label %for.body15

for.cond56.preheader:                             ; preds = %cleanup49, %for.body8
  %za.0.lcssa = phi <mscale x 16 x float> [ %5, %for.body8 ], [ %za.1.lcssa, %cleanup49 ]
  br i1 %or.cond142166, label %cleanup75, label %if.end66.lr.ph

if.end66.lr.ph:                                   ; preds = %for.cond56.preheader
  %add.ptr71 = getelementptr inbounds float, float* %matC, i64 %j.0170
  %6 = call <vscale x 4 x i1> @llvm.aarch64.sve.convert.from.svbool.nxv4i1(<vscale x 16 x i1> %4)
  br label %if.end66

for.body15:                                       ; preds = %for.body15.lr.ph, %cleanup49
  ;CHECK-DAG: zero    {za{{[0-9]+}}.s}
  %k.0160 = phi i64 [ 0, %for.body15.lr.ph ], [ %add52, %cleanup49 ]
  %za.0159 = phi <mscale x 16 x float> [ %5, %for.body15.lr.ph ], [ %za.1.lcssa, %cleanup49 ]
  %7 = call <mscale x 16 x float> @llvm.aarch64.sme.zero.mxv16f32()
  br i1 %or.cond146, label %for.cond30.preheader, label %if.end.lr.ph

if.end.lr.ph:                                     ; preds = %for.body15
  ; CHECK-DAG: whilelo p{{[0-9]+}}.s, x{{[0-9]+}}, x{{[0-9]+}}
  %8 = call <vscale x 4 x i1> @llvm.aarch64.sve.whilelo.nxv4i1.i64(i64 %k.0160, i64 %conv11)
  %9 = call <vscale x 16 x i1> @llvm.aarch64.sve.convert.to.svbool.nxv4i1(<vscale x 4 x i1> %8)
  %add.ptr = getelementptr inbounds float, float* %matA, i64 %k.0160
  %10 = call <vscale x 4 x i1> @llvm.aarch64.sve.convert.from.svbool.nxv4i1(<vscale x 16 x i1> %9)
  br label %if.end

for.cond30.preheader:                             ; preds = %if.end, %for.body15
  %zb.0.lcssa = phi <mscale x 16 x float> [ %7, %for.body15 ], [ %13, %if.end ]
...
if.end40.lr.ph:                                   ; preds = %for.cond30.preheader
  %11 = call <vscale x 4 x i1> @llvm.aarch64.sve.convert.from.svbool.nxv4i1(<vscale x 16 x i1> %2)
  %12 = call <vscale x 4 x i1> @llvm.aarch64.sve.convert.from.svbool.nxv4i1(<vscale x 16 x i1> %4)
  br label %if.end40

if.end:                                           ; preds = %if.end.lr.ph, %if.end
  ; CHECK-DAG: ld1w    {za{{[0-9]+}}h.s[w{{[0-9]+}}, 0]}, p{{[0-9]+}}/z, [x{{[0-9]+}}]
  %add149 = phi i64 [ %i.0174, %if.end.lr.ph ], [ %add, %if.end ]
  %t.0148 = phi i64 [ 0, %if.end.lr.ph ], [ %inc, %if.end ]
  %zb.0147 = phi <mscale x 16 x float> [ %7, %if.end.lr.ph ], [ %13, %if.end ]
  %conv25 = trunc i64 %t.0148 to i32
  %mul = mul i64 %add149, %conv11
  %add.ptr28 = getelementptr inbounds float, float* %add.ptr, i64 %mul
  %13 = call <mscale x 16 x float> @llvm.aarch64.sme.ld1.row.mxv16f32(<vscale x 4 x i1> %10, <mscale x 16 x float> %zb.0147, i32 %conv25, i64 0, float* %add.ptr28)
...
if.end40:                                         ; preds = %if.end40.lr.ph, %if.end40
  ; CHECK-DAG: mov z{{[0-9]+}}.s, p{{[0-9]+}}/m, za{{[0-9]+}}v.s[w{{[0-9]+}}, 0]
  ; CHECK-DAG: ld1w    { z{{[0-9]+}}.s  }, p{{[0-9]+}}/z, [x{{[0-9]+}}]
  ; CHECK-DAG: fmopa   za{{[0-9]+}}.s, p{{[0-9]+}}/m, p{{[0-9]+}}/m, z{{[0-9]+}}.s, z{{[0-9]+}}.s
  %add35156 = phi i64 [ %k.0160, %if.end40.lr.ph ], [ %add35, %if.end40 ]
  %t29.0155 = phi i64 [ 0, %if.end40.lr.ph ], [ %inc48, %if.end40 ]
  %za.1154 = phi <mscale x 16 x float> [ %za.0159, %if.end40.lr.ph ], [ %16, %if.end40 ]
  %conv41 = trunc i64 %t29.0155 to i32
  %14 = call <vscale x 4 x float> @llvm.aarch64.sme.mova.vec.col.nxv4f32.mxv16f32(<vscale x 4 x i1> %11, <mscale x 16 x float> %zb.0.lcssa, i32 %conv41, i64 0)
  %mul44 = mul i64 %add35156, %conv11
  %add.ptr46 = getelementptr inbounds float, float* %add.ptr45, i64 %mul44
  %15 = call <vscale x 4 x float> @llvm.aarch64.sve.ld1.nxv4f32(<vscale x 4 x i1> %12, float* %add.ptr46)
  %16 = call <mscale x 16 x float> @llvm.aarch64.sme.fmopa.mxv16f32.nxv4f32(<vscale x 4 x i1> %11, <vscale x 4 x i1> %12, <mscale x 16 x float> %za.1154, <vscale x 4 x float> %14, <vscale x 4 x float> %15)
...
cleanup49:                                        ; preds = %if.end40, %for.cond30.preheader
  %za.1.lcssa = phi <mscale x 16 x float> [ %za.0159, %for.cond30.preheader ], [ %16, %if.end40 ]
...
if.end66:                                         ; preds = %if.end66.lr.ph, %if.end66
  ; CHECK-DAG: st1w    {za{{[0-9]+}}h.s[w{{[0-9]+}}, 0]}, p{{[0-9]+}}, [x{{[0-9]+}}]
  ...
  %add.ptr72 = getelementptr inbounds float, float* %add.ptr71, i64 %mul70
  call void @llvm.aarch64.sme.st1.row.mxv16f32(<vscale x 4 x i1> %6, <mscale x 16 x float> %za.0.lcssa, i32 %conv67, i64 0, float* %add.ptr72)
...
; Function Attrs: nofree nosync nounwind readnone willreturn
declare <vscale x 4 x i1> @llvm.aarch64.sve.whilelo.nxv4i1.i64(i64, i64) #2
